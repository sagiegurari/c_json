...
void test_impl()
{
  ...
  assert_size_equal(hashtable_size(sub_value->value->object), 1);
  sub_value = hashtable_get(sub_value->value->object, "key");
  assert_true(sub_value->type == JSON_TYPE_STRING);
  assert_string_equal(sub_value->value->string, "value");
